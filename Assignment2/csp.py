...
def calc_domain(problem_size, board_state):
    ...
    while(True):
        ...
        row_domains = []
        for row in xrange(problem_size):
            row_domains.append(row_domain(problem_size, new_state, row))
        
        col_domains = []
        for col in xrange(problem_size):
            col_domains.append(col_domain(problem_size, new_state, col))
        ...
        for i in xrange(block_size):
            block_domain_line = []
            for j in xrange(block_size):
                block_domain_line.append(block_domain(problem_size, new_state, block_size*i, block_size*j))
...
#         print "---------------------calc_domain--------------------"
        for i in xrange(problem_size):
            ...
            for j in xrange(problem_size):
                ...
                if len(domain) == 0:
#                    print "ooni ke khalie   " , i, "-" , j
                    return False , new_state
                elif len(domain) == 1:
#                    print i, j , domain
                    new_state[i][j] = domain[0]
                    changed = True
        ...
        if not input_valid_test(problem_size, new_state):
            return False , board_state    
        if not changed:
            break

    # print "----------------------------------------------------"
    return domains, new_state

def MRV_choose(problem_size, board_state):
#    print "MRV"
    domains, new_state = calc_domain(problem_size, board_state)
    if not domains:
#        print "domaine yeki khaliiii shooooooooooooood!"
#        print "----------------------------------------------------"
#        for boardline in new_state :
#            print boardline
#        print "----------------------------------------------------"
        return -2 , -2, board_state
    ...
    for i in xrange(problem_size):
        for j in xrange(problem_size):
            # print "MRV_choose: ", i ,"-", j ,"------>", len(domains[i][j]) , "miiiiiin = ", min_domain_size
            # print i, "-", j , "~~~~>", domains[i][j]
            if domains[i][j] == [-1]:
                continue
            ...
            if len(domains[i][j]) <= math.sqrt(problem_size):
                return min_row, min_col, new_state
    
    if (min_row == -1) and (min_col == -1):
        if goal_test(problem_size, new_state):
            return min_row, min_col, new_state
        else:
            print "bakhtiiiiiiiiim"
            return -2, -2, board_state
#    print "tahe MRV"
...
#    print "expand"
    result = []
    row,col, new_state = MRV_choose(problem_size, board_state)
    if (row == -2) and (col == -2):
#        print "row o col = -2"
        return result
    if (row == -1) and (col == -1):
...
#    print "expand: ", domain
    ...
    for value in domain:
        super_new_state = copy.deepcopy(new_state)
        super_new_state[row][col] = value
        # satro sotoono box 
                

        # print "new_state",new_state 
        result.append(super_new_state)
        # print "result",result
    return result
